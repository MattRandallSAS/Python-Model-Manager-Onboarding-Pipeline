...
* ### Additional models (optional) ###
   ...
   Up to three more models (pickle file, name, algorithm label each), sharing the same training table, target, and project as the first model.

   ![](img/PythonModelToModelManager-tabAdditionalModels.png)

* ### Model type ###

   Whether the batch is classification or regression/prediction, and (for classification) which class value is the target "event."

   ![](img/PythonModelToModelManager-tabModelType.png)
...
* ### Model Manager project ###
   ...
   ![](img/PythonModelToModelManager-tabProject.png)

* ### Optional steps ###

   Whether to generate a model card, and an optional sensitive column for a bias/fairness assessment (classification only).

   ![](img/PythonModelToModelManager-tabOptional.png)

* ### Publish & performance monitoring (optional) ###
   ...
   ![](img/PythonModelToModelManager-tabPublish.png)
...
* ### Connection ###
   ...
   ![](img/PythonModelToModelManager-tabConnection.png)

* ### About tab ###

   ![](img/PythonModelToModelManager-tabAbout.png)
...
4. Add the **Python Model to Model Manager** step to a flow, point **Model & data** at the pickle and the CSV, name your target column, and run it.
...
`extras/Python to Model Manager - code generator.sas` contains the exact same logic as the custom step, with the UI fields exposed as `%let` statements at the top instead. Fill those in and submit the file directly in a SAS Studio Program tab (or any batch/scheduled SAS session) to run it without importing the custom step — useful for repeated or scripted runs.
